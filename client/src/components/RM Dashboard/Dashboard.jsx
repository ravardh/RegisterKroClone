...
const Dashboard = () => {
  ...
  if (loading) return <div className="p-3 sm:p-4 md:p-6 text-center text-gray-600">Loading your assigned leads...</div>;
  ...
  return (
    <div className="p-3 sm:p-4 md:p-6 bg-white min-h-screen">
      ...
      <div className="mb-4 sm:mb-6">
        ...
        <p className="text-xs sm:text-sm text-gray-500">Overview of your assigned leads and performance</p>
      </div>

      {/* Summary Stats Cards */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 mb-4 sm:mb-6">
        <div className="border border-gray-100 rounded-lg p-3 sm:p-4 hover:border-gray-300 transition">
          <p className="text-xs text-gray-500 uppercase font-medium tracking-wide">Total Leads</p>
          <p className="text-xl sm:text-2xl font-bold text-blue-600 mt-1">{totalLeads}</p>
        </div>
        <div className="border border-gray-100 rounded-lg p-3 sm:p-4 hover:border-gray-300 transition">
          <p className="text-xs text-gray-500 uppercase font-medium tracking-wide">In Progress</p>
          <p className="text-xl sm:text-2xl font-bold text-orange-600 mt-1">{(stageCounts["In Progress"] || 0)}</p>
        </div>
        <div className="border border-gray-100 rounded-lg p-3 sm:p-4 hover:border-gray-300 transition">
          <p className="text-xs text-gray-500 uppercase font-medium tracking-wide">Completed</p>
          <p className="text-xl sm:text-2xl font-bold text-green-600 mt-1">{completedLeads}</p>
        </div>
        <div className="border border-gray-100 rounded-lg p-3 sm:p-4 hover:border-gray-300 transition">
          <p className="text-xs text-gray-500 uppercase font-medium tracking-wide">Completion Rate</p>
          <p className="text-xl sm:text-2xl font-bold text-purple-600 mt-1">{completionRate}%</p>
        </div>
      </div>
      ...
      <div className="border border-gray-100 rounded-lg p-3 sm:p-4 bg-gray-50">
        <h2 className="text-base sm:text-lg font-semibold text-gray-900 mb-3 sm:mb-4">Stage Distribution</h2>
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2 sm:gap-3">
          {stages.map((stage) => (
            <div key={stage} className="text-center p-2 sm:p-3 border border-gray-200 rounded-lg bg-white hover:border-gray-300 transition">
              <p className="text-xl sm:text-2xl font-bold text-gray-900">{stageCounts[stage] || 0}</p>
              <p className="text-xs text-gray-500 mt-1 line-clamp-2">{stage}</p>
            </div>
          ))}
        </div>
      </div>
