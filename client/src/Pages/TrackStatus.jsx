...
const TrackStatus = () => {
  ...
  const handleGetStatus = async (e) => {
    ...
    try {
      ...
      const errorMsg = err.response?.data?.message || "Lead not found. Please check the ID and try again."
      setError(errorMsg)
  ...
  const getCurrentStageIndex = () => {
    if (!trackingData || !trackingData.stages || trackingData.stages.length === 0) {
      return 0
    }
            ...
            <div className="flex flex-col lg:flex-row items-center justify-between gap-8 lg:gap-12 mb-8 lg:mb-12">
                ...
                <div className="bg-white/90 rounded-2xl shadow-2xl p-6 md:p-10 border border-black">
                  <form onSubmit={handleGetStatus} className="space-y-6">
                    <div className="space-y-2">
                      <label className="text-sm font-semibold text-gray-700">Lead ID</label>
                      <input
                        type="text"
                        value={leadId}
              ...
              <section className="text-center lg:text-left space-y-4 rounded-2xl p-6 md:p-8 w-full lg:w-auto">
                ...
                <p className="text-white/85 text-base md:text-lg max-w-xl mx-auto lg:mx-0">
                  Enter your Lead ID to check the latest status of your application instantly.
                </p>
              </section>
            ...
            {error && !trackingData && (
              <section className="w-full mt-8">
                <div className="bg-red-50 rounded-2xl shadow-2xl p-6 md:p-10 border border-red-200">
                  <h2 className="text-xl md:text-2xl font-bold text-red-800 mb-4">Not Found</h2>
            ...
            {trackingData && (
              <section className="w-full mt-8">
                <div className="bg-white/95 rounded-2xl shadow-2xl p-6 md:p-10 border border-white/30">
                  <div className="mb-8">
                    <h2 className="text-xl md:text-2xl font-bold text-gray-800 mb-4">Application Details</h2>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        <p className="text-sm text-gray-600">Lead ID</p>
                        <p className="text-base font-semibold text-gray-900">{trackingData.leadId}</p>
                      </div>
                      <div>
                        <p className="text-sm text-gray-600">Client Name</p>
                        <p className="text-base font-semibold text-gray-900">{trackingData.clientName}</p>
                      </div>
                      <div>
                        <p className="text-sm text-gray-600">Email</p>
                        <p className="text-base font-semibold text-gray-900">{trackingData.clientEmail}</p>
                      </div>
                      <div>
                        <p className="text-sm text-gray-600">Phone</p>
                        <p className="text-base font-semibold text-gray-900">{trackingData.clientPhone}</p>
                      </div>
                      <div>
                        <p className="text-sm text-gray-600">Service</p>
                        <p className="text-base font-semibold text-gray-900">{trackingData.interestedService}</p>
                      </div>
                    </div>
                  </div>

                  <h2 className="text-xl md:text-2xl font-bold text-gray-800 mb-8">Application Journey</h2>
                  ...
                  <div className="relative max-w-2xl mx-auto">
                    ...
                    <div className="space-y-6">
                      {stageOrder.map((stage, index) => {
                        ...
                        const stageData = trackingData.stages.find(s => s.stageName === stage)

                        return (
                          <div key={stage} className="flex gap-4 md:gap-6 items-start relative">
                            {/* Circle marker */}
                            ...
                            <div className="flex-1 pt-2">
                              <div className={`transition-all duration-300 ${isCurrent ? 'bg-blue-50 border-l-4 border-blue-500 pl-4 py-3 rounded' : 'pl-0'}`}>
                                ...
                                {stageData && (
                                  <div className="mt-2 space-y-1">
                                    <p className="text-xs md:text-sm text-gray-600">
                                      <span className="font-semibold">Completed on:</span> {new Date(stageData.updatedAt).toLocaleDateString('en-US', { 
                                        weekday: 'long', 
                                        year: 'numeric', 
                                        month: 'long', 
                                        day: 'numeric' 
                                    ...
                                    <p className="text-xs md:text-sm text-gray-600">
                                      <span className="font-semibold">Time:</span> {new Date(stageData.updatedAt).toLocaleTimeString('en-US', { 
                                        hour: '2-digit', 
                                        minute: '2-digit',
                                        second: '2-digit'
                                      })}
                                    ...
                                    <p className="text-xs md:text-sm text-gray-600">
                                      <span className="font-semibold">Updated by:</span> {stageData.updatedby}
                                    </p>
                                  </div>
                                )}

                                {isCurrent && !stageData && (
                                  <p className="text-sm text-blue-600 font-semibold mt-2 animate-bounce">⏳ Currently in this stage</p>
                                )}
                  ...
                  <div className="mt-8 pt-6 border-t border-gray-200">
                    <div className="flex flex-wrap gap-4">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm text-gray-700">Current Stage</span>
                      </div>
                      <div className="flex items-center gap-2">
                  ...
                  {trackingData.stages && trackingData.stages.length > 0 && (
                    <div className="mt-8 pt-6 border-t border-gray-200">
                      <h3 className="text-lg font-bold text-gray-800 mb-4">Complete Journey Timeline</h3>
                      <div className="space-y-3">
                        {trackingData.stages.map((stage, index) => (
                          <div key={index} className="flex gap-4 p-4 bg-linear-to-r from-green-50 to-white rounded-lg border border-green-200 hover:shadow-md transition-shadow">
                            <div className="text-2xl">{stageIcons[stage.stageName] || '📌'}</div>
                            <div className="flex-1">
                              <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-2">
                                <p className="font-semibold text-gray-800 capitalize">{stageLabels[stage.stageName]}</p>
                                <p className="text-xs md:text-sm text-gray-600">
                                  {new Date(stage.updatedAt).toLocaleString('en-US', { 
                                    year: 'numeric',
                                    month: 'short',
                                    day: 'numeric',
                                    hour: '2-digit',
                              ...
                              <p className="text-sm text-gray-600 mt-1">
                                <span className="font-semibold">Updated by:</span> {stage.updatedby}
                              </p>
                            </div>
                          </div>
                        ))}
